...
class Ensemble():
    ...
    """
    def __init__(self, code: str, data_path: str = None, top_path: str = None, database: str = None, chain_id: str = None, residue_range: Tuple = None, alias: str = None) -> None:
        self.code = code
        ...
        self.alias = code if self.chain_id == None else code + '_' + chain_id
    ...
    def load_trajectory(self, data_dir: str):  
        ...
        if not os.path.exists(self.data_path):
            raise FileNotFoundError(
                f"Data file or directory for ensemble {self.alias} doesn't exist: {self.data_path}"
            )
        elif self.data_path.endswith('.pdb'):
            chain_ids = self.get_chains_from_pdb()
            print(f"{self.alias} chain ids: {chain_ids}")
            
            print(f"Generating trajectory for {self.alias}...")
            ...
            traj_dcd = os.path.join(data_dir, f'{self.alias}{traj_suffix}.dcd')
            traj_top = os.path.join(data_dir, f'{self.alias}{traj_suffix}.top.pdb')
            ...
            print(f"Loading trajectory for {self.alias}...")
        ...
        if self.trajectory.topology.n_chains > 1:
            raise ValueError(f"Multiple chains found for ensemble {self.alias}. "
    ...
    def random_sample_trajectory(self, sample_size: int):
        ...
        self._select_residues()
        print(f"{sample_size} conformations sampled from {self.alias} trajectory.")
    ...
    def extract_features(self, featurization: str, *args, **kwargs):
        ...
        """
        print(f"Performing feature extraction for Ensemble: {self.alias}.")
    ...
    def _select_chain(self, chain_ids: List[str]) -> bool:
        ...
        if self.chain_id is None:
            raise ValueError(f"Multiple chains found in the ensemble {self.alias}. Please specify a chain_id from {chain_ids}.")
        ...
        print(f"Chain {chain_id_upper} selected from ensemble {self.alias}.")
    ...
    def _select_residues(self):
        ...
        print(f"Selected residues from ensemble {self.alias}")
